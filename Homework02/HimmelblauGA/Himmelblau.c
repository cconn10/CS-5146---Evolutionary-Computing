...
double SGA_Himmelblau_Fitness(genome_t *genome)
{
    int x = 0;
    int y = 0;
    ...
    long double himmelblau;

    // get integer values of x and y to plug into the himmelblau function
    ...
    /*
        Fitness = closeness of himmelblau(x,y) to zero
        I chose to add 1 to the himmelblau result because the function minimizes to zero
...
{ 
    ...
    for (m_count = 0; m_count < (*population).member_count; m_count++)
    {
        ...
        ((*population).member[m_count]).fitness = SGA_Himmelblau_Fitness(&((*population).member[m_count]));
    ...
    (*population).diversity = max_distance;
}
...
int main()
{
    ...
    {  
        ...
        cumulative_evaluations += POP_SIZE;

        printf("Himmelblau GA %d %d %.2f %.2f %d %d %lf %lf %.2f\n", POP_SIZE, BIT_STRING_LENGTH, MUATION_RATE, CROSSOVER_RATE, generation_count + 1, (*(&POPULATION)).champion_fitness, (*(&POPULATION)).average_fitness, (*(&POPULATION)).diversity);
    }

    if((*(&POPULATION)).diversity != 0.0)
